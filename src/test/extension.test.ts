...
async function findTestContainer(): Promise<string | undefined> {
	...
	const own = await dockerPsIds(['--filter', `label=devcontainer.local_folder=${repoRoot}`]);
	if (own.length > 0) {
		return own[0];
	}
...
async function assertFsError(promise: Promise<unknown>, code: string): Promise<void> {
	try {
		await promise;
	} catch (err) {
		assert.ok(err instanceof vscode.FileSystemError, `expected FileSystemError(${code}), got: ${err}`);
...
suite('DevContainerFileSystemProvider (live container)', function () {
	...
	suiteSetup(async function () {
		...
		await provider.writeFile(u(`${root}/hello.txt`), Buffer.from('hello container fs\n'), { create: true, overwrite: true });
	});
	...
	suiteTeardown(async function () {
		...
		await provider.delete(u(root), { recursive: true }).catch(() => { /* best effort */ });
	});
	...
	test('readDirectory: missing dir throws FileNotFound', async () => {
		await assertFsError(provider.readDirectory(u(`${root}/missing`)), 'FileNotFound');
	});

	test('readFile returns contents', async () => {
		const content = await provider.readFile(u(`${root}/hello.txt`));
		assert.strictEqual(Buffer.from(content).toString('utf8'), 'hello container fs\n');
	});
	...
	test('writeFile: create=false on missing file throws FileNotFound', async () => {
		await assertFsError(provider.writeFile(u(`${root}/new.txt`), new Uint8Array(0), { create: false, overwrite: true }), 'FileNotFound');
	});

	test('writeFile: create without overwrite on existing file throws FileExists', async () => {
		await assertFsError(provider.writeFile(u(`${root}/hello.txt`), new Uint8Array(0), { create: true, overwrite: false }), 'FileExists');
	});

	test('writeFile overwrites and reads back', async () => {
		await provider.writeFile(u(`${root}/hello.txt`), Buffer.from('updated'), { create: true, overwrite: true });
		const content = await provider.readFile(u(`${root}/hello.txt`));
	...
	test('writeFile: binary roundtrip', async () => {
		...
		await provider.writeFile(u(`${root}/bin.dat`), bytes, { create: true, overwrite: true });
	...
	test('rename moves a file', async () => {
		await provider.writeFile(u(`${root}/mv-src.txt`), Buffer.from('x'), { create: true, overwrite: true });
		await provider.rename(u(`${root}/mv-src.txt`), u(`${root}/mv-dst.txt`), { overwrite: false });
	...
	test('rename: existing target without overwrite throws FileExists', async () => {
		await provider.writeFile(u(`${root}/rn-a.txt`), Buffer.from('a'), { create: true, overwrite: true });
		await assertFsError(provider.rename(u(`${root}/rn-a.txt`), u(`${root}/hello.txt`), { overwrite: false }), 'FileExists');
	});

	test('delete: missing file throws FileNotFound', async () => {
		await assertFsError(provider.delete(u(`${root}/missing.txt`), { recursive: false }), 'FileNotFound');
	});

	test('delete: recursive removes a directory tree', async () => {
		await provider.createDirectory(u(`${root}/doomed`));
		await provider.writeFile(u(`${root}/doomed/f.txt`), Buffer.from('x'), { create: true, overwrite: true });
	...
	test('filenames with spaces roundtrip', async () => {
		await provider.writeFile(u(`${root}/a file with spaces.txt`), Buffer.from('spaces'), { create: true, overwrite: true });
		const entries = await provider.readDirectory(u(root));
	...
	test('provider is registered with vscode.workspace.fs', async () => {
		...
		assert.ok(names.includes('tmp'), `expected tmp in container root, got: ${names.join(', ')}`);
